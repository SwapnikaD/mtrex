...
class SimilarityCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        targets_cosine_thresholded = targets > cosine_embedding_loss_margin

        loss_cosine = F.cosine_embedding_loss(logits0, logits1, targets, margin=cosine_embedding_loss_margin,
                                              reduction='sum')
        ...
        preds_cosine_thresholded = torch.cosine_similarity(logits0, logits1,
                                                           dim=1) > cosine_embedding_loss_margin
