...
class TrexEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        embed_dim = embed_tokens[static_field].embedding_dim

        # assume the padding will be the same for a sequences,
        self.padding_idx = embed_tokens[static_field].padding_idx
        ...
        self.byte_fields: Dict[str, torch.Tensor] = {byte_field: torch.tensor([]) for byte_field in byte_fields}
    ...
    def forward_embedding(self, src_tokens: Dict[str, torch.Tensor]):
        if self.seq_agg is not None:
            token_embeddings = []
            # embed tokens (static)
            token_embeddings.append(self.embed_tokens[static_field](src_tokens[static_field]))
            ...
            if self.drop_field != 'static':
                token_embedding = self.embed_tokens[static_field](src_tokens[static_field])
            else:
    ...
    def forward_scriptable(
        ...
        encoder_padding_mask = src_tokens[static_field].eq(self.padding_idx)  # padding is from byte sequences
        has_pads = (src_tokens[static_field].device.type == "xla" or encoder_padding_mask.any())
        ...
        src_lengths = src_tokens[static_field] \
...
class ByteCombineCNN(nn.Module):
    def __init__(self, embed_dim, output_dim, activation_fn='relu',
                 filters=[(1, 4), (2, 8), (3, 12)] + [(i, 4 * i) for i in range(4, byte_len)],
                 highway_layers=2):
